...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/recipes/:type", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: `Failed to fetch ${req.params.type} recipes` });
    }
  });
  ...
  app.post("/api/mealplan", async (req, res) => {
    try {
      ...
      if (!result.success) {
        return res
          .status(400)
          .json({ error: "Invalid meal plan data", details: result.error });
      }

      const { breakfastIds, dinnerIds } = result.data;

      console.log("Breakfast IDs:", breakfastIds);
      console.log("Breakfast IDs type:", Array.isArray(breakfastIds) ? "array" : typeof breakfastIds);
      console.log("Dinner IDs:", dinnerIds);
      console.log("Dinner IDs type:", Array.isArray(dinnerIds) ? "array" : typeof dinnerIds);
      ...
      const mealPlan = await dbStorage.generateMealPlan(
        breakfastIds,
        dinnerIds,
      );
  ...
  app.post("/api/mealplan/save", async (req, res) => {
    try {
      ...
      console.log("Saving meal plan with:");
      console.log("- Breakfast IDs:", breakfastIds);
      console.log("- Dinner IDs:", dinnerIds);
      console.log("- Days:", days);

      const savedMealPlan = await dbStorage.saveMealPlan(
        breakfastIds,
        dinnerIds,
        days,
      );
  ...
  app.post("/api/grocerylist", async (req, res) => {
    try {
      ...
      if (!result.success) {
        return res
          .status(400)
          .json({ error: "Invalid grocery list data", details: result.error });
      }
